...
def go_there():
    ...
    N_coords = float(test_txt1.get())
    max_iter = int(test_txt2.get())
    date = str(date_txt.get())
    ...
    ##PRINT TRIAL COORDINATES:
    print("X_COORD_LIST: ", x_coord_list)
    print("Y_COORD_LIST: ", y_coord_list)
    
    ##GET CALIBRATION DATA
    m1 = slope1[0]
    m2 = slope2[0]
    
    ##CONFIGURE LISTS FOR DATA TRACKING
    xlistpoints.clear()
    ylistpoints.clear()
    jlist.clear()
    v_inputs.clear()
    ...
    while j < len(x_coord_list):
        print("BACK TO TOP OF OUTER LOOP")
        xf = x_coord_list[j]
        yf = y_coord_list[j]
        print('J: ', j)
        ...
        i = 0
        imagename = "go_there_" + str(date) + "_j" + str(j) + "i" + str(i)
        DoPhotoCopy2.takePhotoAndCopy(x,imagename)
        imagename=imagename+".jpg"
        while os.path.exists(imagename) == False:
            imagename = "go_there_" + str(date) + "_j" + str(j) + "i" + str(i)
        ...
        while i < (max_iter):
            ...
            if i == max_iter:
                j+=1
                print('FINAL ITERATION, INCREMENTED J')
            else:
                print('ITERATION: ', i)
            delta_x = xf - xi
            delta_y = yf - yi
            ...
            ### LOCATE TIP AGAIN!
            # take photo
            imagename = "go_there_" + str(date) + "_j" + str(j) + "i" + str(i) + 'CH1'
            DoPhotoCopy2.takePhotoAndCopy(x,imagename)
            imagename=imagename+".jpg"
            while os.path.exists(imagename) == False:
                imagename = "go_there_" + str(date) + "_j" + str(j) + "i" + str(i) + 'CH1'
    ...
    df = pd.DataFrame(list(zip(*[jlist, xlistpoints, ylistpoints, v_inputs, axis_inputs, dir_inputs, steps_inputs]))).add_prefix('Col')
    df.columns = ['J', 'X_pix', 'Y_pix', 'V_input', 'Ax_input', 'Dir_input', 'Steps_input']
    print("DF: ", df)
    ### MIGHT WANT TO ADD TEXT BOX TO DEFINE CSV NAME AT A LATER STATE
...
lblfrt1=tkinter.Label(top,text = "N_coords:")
...
lblfrt3.grid(row=10, column=2)

frt = tkinter.Button(top, text="TEST 'GO THERE'", command=go_there)
frt.grid(row = 11, column = 3)
